...
def clean_html_smart(html_content: str):
    ...
    # Strip reference tags and text citation brackets completely
...
def generate_ai_summary(paragraphs: list, sentences_count: int = 3) -> list:
    ...
    # Simple word tokenizer matrix
    words = re.findall(r'\b\w{4,15}\b', all_text) # Target meaningful words
    if not words:
    ...
    # Segment back into concrete summary candidate sentences
    sentences = []
    for p in paragraphs:
        # Split sentences by period boundary lines
        split_sentences = re.split(r'(?<=[.!?])\s+', p)
        for s in split_sentences:
            if len(s.strip()) > 30:
                sentences.append(s.strip())
                
    # Rank sentences based on local weights
    sentence_scores = {}
    ...
    # Sort and harvest top core briefs
...
def create_native_pdf(title: str, source_url: str, paragraphs: list, font_size: int, show_summary: bool, ai_bullets: list) -> bytes:
    ...
    # 🌟 INJECT AI SUMMARY MATRIX IN LAYOUT
    if show_summary and ai_bullets:
        lines.append("=========================================================================")
...
# 🌟 NEW CONTROL: Toggle AI summary mode / 控制開關：開啟 AI 摘要
ui_enable_summary = st.sidebar.checkbox("Activate AI Core Summarizer / 啟用 AI 自動核心摘要", value=True)
summary_length = st.sidebar.slider("Summary Bullet Count / 摘要句數:", min_value=2, max_value=5, value=3)
    ...
    if st.button("🚀 Process & Summarize Content", type="primary"):
        with st.spinner("⚡ AI Core Processing: Fetching layout payload and running matrix analytics..."):
            try:
                ...
                # 🌟 RUN THE AI MATRIX SUMMARY PIPELINE
                ai_summary_points = []
                if ui_enable_summary:
                    ai_summary_points = generate_ai_summary(clean_paragraphs, summary_length)
